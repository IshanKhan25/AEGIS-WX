...
class AegisPipeline:
    ...
    def run(self, seed: int | None=None, regime: str | None=None, mode: str="DEMO", scenario: str | None=None) -> dict:
        started=time.perf_counter(); scenario, data_mode=load_scenario(self.config,mode,seed=seed,regime=regime,scenario=scenario)
        ...
        for variable in ("precipitation","temperature_2m","wind_speed_10m"):
            ...
            safety=SafetyFallback(threshold); final, fallback, status=safety.apply(corrected,members,weights,unc)
            outputs[variable]={"weights":weights,"raw_blended":blended,"bias_corrected":corrected,"final":final,"uncertainty":unc,"confidence":conf,"fallback_mask":fallback,"fallback_status":status,"fallback_reasons":safety.last_reasons,"fallback_fraction":safety.last_fraction,"members":members,"exceedance_probability":DisagreementUncertainty().exceedance_probability(members,self.config["thresholds"]["heavy_rain"] if variable=="precipitation" else self.config["thresholds"]["severe_wind"])}
