...
class RAMSTABlender:
    ...
    def weights(self, forecasts: dict[str, dict[str,np.ndarray]], variable: str, regime: str, leads: np.ndarray) -> np.ndarray:
        ...
        mean = np.nanmean(stack, axis=1, keepdims=True); disagreement = np.abs(stack-mean)
        scale = np.maximum(np.nanstd(stack,axis=1,keepdims=True), .25)
        ...
        logits = np.where(np.isfinite(stack), logits, -1e9)
        logits -= logits.max(axis=1,keepdims=True); exp = np.exp(logits)
        return exp / np.maximum(exp.sum(axis=1,keepdims=True), 1e-12)
    def blend(self, forecasts: dict[str, dict[str,np.ndarray]], variable: str, weights: np.ndarray) -> np.ndarray:
        stack=np.stack([forecasts[m][variable] for m in MODELS],axis=1); return np.nansum(stack*weights,axis=1)
