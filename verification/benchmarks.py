...
def benchmark(result: dict, variable: str="precipitation") -> pd.DataFrame:
    ...
    rows=[]
    for name,pred in candidates.items(): rows.append({"Model":name,"RMSE":rmse(pred,truth),"MAE":mae(pred,truth),"Bias":bias(pred,truth),"Correlation":correlation(pred,truth),"CSI":csi(pred,truth,threshold),"ROC-AUC":roc_auc(pred,truth,threshold)})
